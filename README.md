...
`·` は未割り当て(KC_NO)。`1 !` のような 2 文字表記は「通常時 / Shift 押下時」(US 配列)。Layer 1 は左親指の `MO(1)` を押している間だけ有効。`BT0`〜`BT2` は Bluetooth 接続先の切り替え(`QK_KB_0`〜`QK_KB_2`)。
...
│ LSft │  Z  │  X  │  C  │  V  │  B  │      │  N  │  M  │ , < │ . > │  ↑  │ / ?  │
├──────┼─────┼─────┼─────┼─────┼─────┤      ├─────┼─────┼─────┼─────┼─────┼──────┤
│ Ctrl │ Gui │ Alt │MO(1)│ Cmd │ Spc │      │ Spc │ Cmd │ Sft │  ←  │  ↓  │  →   │
└──────┴─────┴─────┴─────┴─────┴─────┘      └─────┴─────┴─────┴─────┴─────┴──────┘
```
